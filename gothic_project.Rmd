...
```{
messy_frankenstein <- frankenstein %>%
  unnest_tokens(word, text)

##frankenstein_sentiment <- messy_frankenstein %>%
##  inner_join(get_sentiments("bing")) %>%
##  count(word, sentiment) %>%
##  spread(sentiment, n, fill = 0) %>%
##  mutate(sentiment = positive - negative)

frankenstein_sentiment <- tidy_frankenstein %>%
  inner_join(get_sentiments("bing")) %>%
  count(sentences, sentiment) %>%
  spread(sentiment, n, fill = 0) %>%
  mutate(sentiment = positive - negative)

ggplot(frankenstein_sentiment, aes(sentences, sentiment)) +
...
ggplot(jekyllhyde_sentiment, aes(word, sentiment)) +
  geom_col(show.legend = FALSE, color = "red") 

```

Okay, this is a whole mess right now. i want to map pronoun usage and negative sentiment across the books to see if I can track monstrosity, but I genuinely have no idea how to do all of that. So, I am frustrated.
...
jekyllhyde_sentiment <- tidy_jekyllhyde %>%
  inner_join(get_sentiments("bing")) %>%
  count(word, sentiment) %>%
  spread(sentiment, n, fill = 0) %>%
  mutate(sentiment = positive - negative)
...
I have also decided to make word clouds depicing the most frequently used words in all of the books. 
