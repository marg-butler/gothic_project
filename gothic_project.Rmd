...
neat_no_name <- no_names %>% 
  mutate(carm = ifelse(title == "Carmilla", "yes", "no")) %>%
  mutate(drac = ifelse(title == "Dracula", "yes", "no")) %>% 
  mutate(fran = ifelse(title == "Frankenstein; Or, The Modern Prometheus", "yes", "no")) %>%
  mutate(phan = ifelse(title == "The Phantom of the Opera", "yes", "no")) %>%
  mutate(jeky = ifelse(title == "The Strange Case of Dr. Jekyll and Mr. Hyde", "yes", "no")) 
...
```
```{r}
cloud_gothic <- no_names

write_rds(cloud_gothic, "gothiclit/cloud_gothic.rds")
